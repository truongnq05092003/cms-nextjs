...
import { getTranslator } from "@/lib/settings";
...
export default function LoginPage() {
	...
	const val = () => {
		...
		if (isNullOrEmpty(email) || isNullOrEmpty(email.trim())) {
			hasErr = true;
			setEmailErr(t("auth.err_email_required"));
		} else if (!isEmail(email)) {
			hasErr = true;
			setEmailErr(t("auth.err_email_valid"));
		}

		if (isNullOrEmpty(password) || isNullOrEmpty(password.trim())) {
			hasErr = true;
			setPasswordErr(t("auth.err_password_required"));
		} else if ((password.trim() || []).length < 7) {
			hasErr = true;
			setPasswordErr(t("auth.err_password_limit_6"));
		}
	...
	return (
		<div className="px-6 py-8 rounded-[20px] w-[calc(100vw-48px)] md:w-[500px] shadow backdrop-blur">
			<form className="relative w-full h-full z-20">
				...
				<div className="mt-8 w-full flex justify-center ">
					<Button
						type="button"
						className="w-full px-10 h-11 bg-white text-[#000000] hover:bg-white"
